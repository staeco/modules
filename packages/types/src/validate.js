...
const validateField = (path, value) => {
  ...
  if (value.notes) {
    ...
    if (typeof value.notes === 'string' && value.notes.length > MAX_LENGTH) errors.push({ value: value.notes, path: [ ...path, 'notes' ], message: `Must be less than ${MAX_LENGTH} characters` })
  }
  ...
  if (value.items) validateField([ ...path, 'items' ], value.items)

  // validation
  if (value.validation && typeof value.validation !== 'object') errors.push({ value: value.validation, path: [ ...path, 'validation' ], message: 'Not a valid object value' })
  ...
  // measurement
  if (value.measurement && typeof value.measurement !== 'object') errors.push({ value: value.measurement, path: [ ...path, 'measurement' ], message: 'Not a valid object value' })
...
export default (dataType) => {
  ...
  if (dataType.schema && typeof dataType.schema !== 'object') errors.push({ value: dataType.schema, path: [ 'schema' ], message: 'Not a valid object value' })
  if (dataType.schema) {
